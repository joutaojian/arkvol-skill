...
安装器会自动识别当前 Runtime 并安装到对应目录。需要指定 Runtime 时，可添加 `-a claude-code`、`-a codex`、`-a cursor` 或 `-a openclaw` 等参数。

### 方式三：手动安装

将仓库克隆到所用 Runtime 的 Skills 目录：

| Runtime | 安装路径 |
| --- | --- |
| Claude Code | `~/.claude/skills/arkvol-skill/` |
| Codex CLI | `~/.codex/skills/arkvol-skill/` |
| Cursor | `~/.cursor/skills/arkvol-skill/` |
| OpenClaw | `~/.openclaw/workspace/skills/arkvol-skill/` |
| 其他 Runtime | 对应 Runtime 的 `skills/` 目录 |

```bash
git clone https://github.com/joutaojian/arkvol-skill <对应的安装路径>
```
