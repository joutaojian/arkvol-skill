...
使用 Arkvol 提供的 6 个聚合数据接口获取市场情绪、完整页面明细和解释文字。Arkvol 是覆盖 A 股、港股和美股的金融数据分析服务。
...
需要完整分析时使用 `--json`。必须保留响应中的 `page_text` 和 `original_page_data`；用 `page_text` 解释指标、计算口径和风险边界，不要仅凭字段名推断。
